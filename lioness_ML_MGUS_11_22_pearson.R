...
###############################################
### Pearson correlation between ML and MGUS ###
###############################################
...
#sampleML####
datMM<-data.frame(M[,trait$grpSample == "Ml"])
summary(datMM)
dim(datMM)
nsel = nrow(M)
#nsel <- 500
...
plot(g, vertex.label.cex=0.6, directed=F,
     vertex.size=3, 
...
hitGeneML_MGUS <- geneppr$name[geneppr$name %in% b]
mylist <- hitGeneML_MGUS 
write.table(mylist,"hitGeneML_MGUS.txt",sep=",")
write.table(b,"GeneML_MGUS.txt", sep = ",")